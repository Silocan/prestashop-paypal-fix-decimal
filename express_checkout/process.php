...
class PaypalExpressCheckout extends Paypal
{
    ...
    private function setProductsList(&$fields, &$index, &$total)
    {
        foreach ($this->product_list as $product) {
            ...
            $fields['L_PAYMENTREQUEST_0_AMT'.$index] = Tools::ps_round($product['price'], $this->decimals); //change to fix tva error
            $fields['L_PAYMENTREQUEST_0_QTY'.$index] = $product['quantity'];

            $total = $total + ($fields['L_PAYMENTREQUEST_0_AMT'.$index] * $product['quantity']);
        }
      
        $summary = $this->context->cart->getSummaryDetails();  //add to fix tva error
        $fields['PAYMENTREQUEST_0_TAXAMT'] = Tools::ps_round($summary['total_tax'], $this->decimals);  //add to fix tva error
    }
    ...
    private function setDiscountsList(&$fields, &$index, &$total)
    {
        ...
        if (count($discounts) > 0) {
            foreach ($discounts as $discount) {
                ...
                $fields['L_PAYMENTREQUEST_0_AMT'.$index] = -1 * Tools::ps_round($discount['value_tax_exc'], $this->decimals); //edit to fix tva error
    ...
    private function setPaymentValues(&$fields, &$index, &$total, &$taxes)
    {
        if (version_compare(_PS_VERSION_, '1.5', '<')) {
            ...
            $shipping_cost_wt = $this->context->cart->getTotalShippingCost(null, false); //edit to fix tva error
        }
        ...
        if ($total <= 1) {
            ...
            $fields['PAYMENTREQUEST_0_AMT'] = $total + Tools::ps_round($summary['total_tax'], $this->decimals) + Tools::ps_round($shipping_cost_wt, $this->decimals); //edit to fix tva error
        } else {
            if ($currency->iso_code == 'HUF') {
                ...
                $fields['PAYMENTREQUEST_0_ITEMAMT'] = Tools::ps_round($total, $this->decimals);
                $fields['PAYMENTREQUEST_0_AMT'] = sprintf('%.2f', ($total + Tools::ps_round($summary['total_tax'], $this->decimals) + $fields['PAYMENTREQUEST_0_SHIPPINGAMT'])); //edit to fix tva error
            }
        }
    }
    ...
    public function getTotalPaid()
    {
        ...
        foreach ($this->product_list as $product) {
            $price = Tools::ps_round($product['price'], $this->decimals);  //edit to fix tva error
            $quantity = Tools::ps_round($product['quantity'], $this->decimals);
            $total = Tools::ps_round($total + ($price * $quantity), $this->decimals);
        }
        $summary = $this->context->cart->getSummaryDetails();  //add to fix tva error
        $total += Tools::ps_round($summary['total_tax'], $this->decimals);  //add to fix tva error
        ...
        if (version_compare(_PS_VERSION_, '1.5', '<')) {
            ...
            $shipping_cost = $this->context->cart->getTotalShippingCost(null, false); //edit to fix tva error
        }

        if (count($discounts) > 0) {
            foreach ($discounts as $product) {
                $price = -1 * Tools::ps_round($product['value_tax_exc'], $this->decimals); //edit to fix tva error
